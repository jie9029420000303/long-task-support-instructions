---
name: lt-ui-tester-high
description: 長任務介面子代理（sonnet／effort high，錯 2 後重派檔）。只由 long-task-orchestrator 主線依狀態檔派工、不自動選用：按案例用真實點擊／輸入操作前台並截圖蒐證，回報 PASS／FAIL／BLOCKED，不改程式、案例或設計。
model: sonnet
effort: high

...
保存：原始目標、最新核准變更、驗收標準、候選全集（找 X／確認沒有 X 型任務：維度取值、清單、候選／已驗／未驗計數）、C 研究證據（研究／系統規劃任務：研究問題、來源類型、截止時點，以及主線核對過的議題／主張、決策問題、原始來源、支持／反證、適用範圍、未證實事項）、目前階段與停止點；模型／effort 鎖定（角色、`subagent_type`、當時解析到的實際模型、時間）；每個工作包的 id、類型、負責 Agent 定義、依賴、候選版號／雜湊、狀態、錯誤次數、目前 effort 檔、成果與證據位置；每批並行派工的範圍比對結果（「並行批次」節）；主線自做的步驟與理由（「主線自做」節）；PASS／FAIL 判定紀錄。
...
- **並行派工閘**：同一批要並行的包，連同所有在途包，先跑
  ...
  exit 0＝無交集，把輸出貼進狀態檔「並行批次」後才在同一則回覆內並行派出；exit 1＝列出交集項與涉及的包，後派的加依賴改依序或把共用項切給唯一一包（另一包 prompt 明寫「不改該項，需要時寫進回報待決」），改完重跑；exit 2＝prompt 檔缺 `[可修改範圍]` 或 `[共用資源]` 段，補上再跑。script 比對：路徑（含 `**`／`*` 前綴涵蓋與無目錄前綴的檔名）＋ `[共用資源]` 裡所有 `鍵:值` 標記（同名即交集；`瀏覽器:playwright` 兩包同寫就擋，C 研究包的 `議題:<slug>` 同名也擋）。
...
[判準] 客觀可核對的 PASS 條件
[可修改範圍] 檔案／目錄清單
[共用資源] 會觸碰的：套件:安裝（預設不准）、migration:<主線配發的編號>、服務:<port 或名稱>、DB:<名稱>、資料:<測試資料集>、帳號:<登入帳號>、瀏覽器:pane|playwright|devtools（介面包必填）、議題:<slug>（C 研究包必填）；沒有就寫「無」
[資料副作用]；[禁止事項]；[隔離／還原方式]
...
[可修改範圍] 只有 <狀態目錄>/evidence/<id>/**（來源快照；寫 ** 才受閘比對）；[共用資源] 預設「議題:<slug>」，JS 渲染頁才加 瀏覽器:pane

...
保存：原始目標、最新核准變更、驗收標準、目前階段與停止點；模型／effort 鎖定（角色、`subagent_type`、當時解析到的實際模型、時間）；每個工作包的 id、類型、負責 Agent 定義、依賴、候選版號／雜湊、狀態、錯誤次數、目前 effort 檔、成果與證據位置；每批並行派工的範圍比對結果（「並行批次」節）；主線自做的步驟與理由（「主線自做」節）；PASS／FAIL 判定紀錄。
